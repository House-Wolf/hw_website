...
import { getAssistanceRequests } from "@/lib/packtracker/assistance";
import PTAssistanceRequestCard from "@/components/pack-tracker/PTAssistanceRequestCard";
import PTEmptyState from "@/components/pack-tracker/PTEmptyState";
import { HelpCircle, Plus } from "lucide-react";

export const metadata = { title: "Assistance Hub — PackTracker" };
...
export default async function AssistancePage({
  ...
  const requests = await getAssistanceRequests({
    ...
    kind: sp.kind as any ?? undefined,
  });

  const statuses = ["OPEN", "ACCEPTED", "IN_PROGRESS", "COMPLETED", "CANCELLED", "REFUSED"];
    ...
    <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm" style={{ color: "var(--text-muted)" }}>
              Request or offer help with missions, mining, combat & more
            </p>
          </div>
        </div>
        <Link
          href="/pack-tracker/assistance/new"
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {statuses.map((s) => (
          <Link
            key={s}
            href={`/pack-tracker/assistance?status=${s}`}
            className="px-3 py-1 rounded-full text-xs font-semibold border transition-colors"
            style={{
      ...
      {requests.length === 0 ? (
        <PTEmptyState
          ...
          description="Be the first to post a request for help."
          actionHref="/pack-tracker/assistance/new"
          actionLabel="Post Request"
        />
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {requests.map((r) => (
            <PTAssistanceRequestCard key={r.id} request={r as any} />
          ))}
        </div>
      )}
    </div>
