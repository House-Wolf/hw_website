...
export default function MarketplaceHero(): JSX.Element {
  ...
  useEffect(() => {
    ...
    const triggerGlitch = () => {
      setShowGlitch(true);
      setTimeout(() => setShowGlitch(false), 10000); // Glitch lasts 10 seconds
    };

    const scheduleNextGlitch = () => {
      // Random interval between 5-10 minutes (300000ms - 600000ms)
      const nextGlitchDelay = 300000 + Math.random() * 300000;

      setTimeout(() => {
        triggerGlitch();
        scheduleNextGlitch(); // Schedule the next one after this completes
      }, nextGlitchDelay);
    };

    // Initial delay before first glitch (1-3 minutes)
